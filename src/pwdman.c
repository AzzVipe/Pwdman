...
{
	char sql[BUFFSIZE];
	const char sql_buf[] = "INSERT INTO " TABLE " (site, email, password) VALUES('%s', '%s', '%s')";
...
{
	char sql[BUFFSIZE];
	const char sql_buf[] = "UPDATE " TABLE " SET (site, email, password) = ('%s', '%s', '%s') WHERE id = '%d'";
...
{
	database_create_app ();
	// No validation is needed here as the validation is done
	// in request handler

	// Save new login to database
	struct pwdman temp = {0};
...
bool pwdman_delete(struct request *req)
{
	int id;
	char sql[BUFFSIZE];
	const char sql_buf[] = "DELETE FROM " TABLE " WHERE id = %d";
...
{
	char sql[BUFFSIZE];
	const char sql_buf[] = "SELECT * FROM " TABLE " WHERE email LIKE '%%%s%%' ";
...
{
	char sql[BUFFSIZE];
	const char sql_buf[] = "SELECT * FROM " TABLE " WHERE site LIKE '%%%s%%' ";
...
{
	char sql[BUFFSIZE];
	const char sql_buf[] = "SELECT * FROM " TABLE " WHERE id=%d";
